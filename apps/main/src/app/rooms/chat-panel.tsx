...
import type { ActiveRoom } from "./room-view";
...
export function ChatPanel({ activeRoom, onBackToGlobal }: ChatPanelProps) {
  ...
  const { messages: liveMessages, status, send } = useChatSocket(cid, ready && me != null);
  ...
  useLayoutEffect(() => {
    ...
    const shouldStick =
      isFirstRender.current || forceScrollRef.current || distanceFromBottom < 120;
  ...
  return (
    <aside className="pointer-events-auto flex h-full w-96 flex-col overflow-hidden rounded-2xl border border-white/40 bg-white/70 shadow-2xl shadow-zinc-900/10 backdrop-blur-xl">
      ...
      <div ref={scrollRef} className="flex flex-1 flex-col overflow-y-auto overflow-x-hidden px-5 py-4">
        <div className="mt-auto space-y-3">
          ...
          {allMessages.map((m) => {
            if (m.type === "SYSTEM") {
              return (
                <p key={m.id} className="py-1.5 text-center text-xs text-zinc-500">
                  {m.content}
                </p>
              );
            }
